...
// Given steps
Given('I have the Result utilities imported', function (this: ErrorHandlingWorld) {
  this.set('ok', ok)
  this.set('err', err)
})
...
Given('I have an err result with an error', function (this: ErrorHandlingWorld) {
  this.set('result', err(errors.notFound('Resource')))
})

Given('I have an err result with a NotFoundError', function (this: ErrorHandlingWorld) {
  this.set('result', err(errors.notFound('Resource')))
})

// When steps
When('I create an ok result with value {string}', function (this: ErrorHandlingWorld, value: string) {
  this.set('result', ok(value))
})

When('I create an err result with a NotFoundError', function (this: ErrorHandlingWorld) {
  this.set('result', err(errors.notFound('Resource')))
})
...
When('I map the result to double the value', function (this: ErrorHandlingWorld) {
  const result = this.get('result')
  this.set('mappedResult', map(result, (value: any) => String(Number(value) * 2)))
})

When('I flatMap the result to fetch user details', function (this: ErrorHandlingWorld) {
  const result = this.get('result')
  this.set(
    'mappedResult',
...
// Then steps
Then('the result should have ok equal to true', function (this: ErrorHandlingWorld) {
  const result = this.get('result')
  expect(result.ok).to.be.true
})

Then('the result value should be {string}', function (this: ErrorHandlingWorld, expectedValue: string) {
  const result = this.get('result')
  if (result.ok) {
...
Then('the result should have ok equal to false', function (this: ErrorHandlingWorld) {
  const result = this.get('result')
  expect(result.ok).to.be.false
})

Then('the result error should be a NotFoundError', function (this: ErrorHandlingWorld) {
  const result = this.get('result')
  if (!result.ok) {
...
Then('I should get the value {string}', function (this: ErrorHandlingWorld, expectedValue: string) {
...
Then('the result should be ok with value {string}', function (this: ErrorHandlingWorld, expectedValue: string) {
...
Then('the result should be ok with user data', function (this: ErrorHandlingWorld) {
  const result = this.get('mappedResult')
  expect(result.ok).to.be.true
  if (result.ok) {
...
Then('the error should be a NotFoundError', function (this: ErrorHandlingWorld) {
  const result = this.get('mappedResult')
  if (!result.ok) {
    expect(result.error.code).to.equal('NOT_FOUND')
  }
